...
=======
        ...
        { id: 33, url: 'https://res.cloudinary.com/dbgbuzyw8/image/upload/v1753360855/Womens_Day_Poster_ocekcs.jpg', title: 'Product Launch' },
        { id: 34, url: 'https://res.cloudinary.com/dbgbuzyw8/image/upload/v1753360853/Free_Bird_Training_Poster_01_s07qmi.jpg', title: 'Brand Campaign' },
        { id: 35, url: 'https://res.cloudinary.com/dbgbuzyw8/image/upload/v1753360852/Banner_04__ybhocf.jpg', title: 'Marketing Design' },
        { id: 36, url: 'https://res.cloudinary.com/dbgbuzyw8/image/upload/v1753360845/SaveVid.Net_448289640_122119497950291490_3216005744129623178_n_zhkcu9.jpg', title: 'Social Media Ad' },
        { id: 37, url: 'https://res.cloudinary.com/dbgbuzyw8/image/upload/v1753360844/SaveVid.Net_441272846_354665350930168_880583373601091162_n_an6haw.jpg', title: 'Event Poster' },
        { id: 38, url: 'https://res.cloudinary.com/dbgbuzyw8/image/upload/v1753360850/Free_Bird_poster_c35cce.jpg', title: 'Product Launch' },
        { id: 39, url: 'https://res.cloudinary.com/dbgbuzyw8/image/upload/v1753360845/SaveVid.Net_451383399_122126568632291490_5314762256159234429_n_zfwha2.jpg', title: 'Brand Campaign' },
...
=======
        { id: 46, url: 'https://physioheal-pro.vercel.app', image: 'https://www.revivept.ca/client/1686/media/img/430337/Physiotherapy_Services_in_Edmonton_AB.jpg', title: 'Healthcare Website' },
        { id: 47, url: 'https://snehasafetynets.com', image: 'https://www.manusafetynets.com/assets/img/image/services/balcony-nets.jpg', title: 'Safety Nets Website' },
        { id: 48, url: 'https://udayantra.netlify.app', image: 'https://mir-s3-cdn-cf.behance.net/projects/404/bf1e68214791583.Y3JvcCwzMDY4LDI0MDAsNjgsMA.jpg', title: 'Portfolio Website' },
        { id: 49, url: 'https://gorgeous-boba-3ba51c.netlify.app', image: 'https://res.cloudinary.com/dbgbuzyw8/image/upload/v1753359024/8aug_mqqnry.png', title: 'Agency Website' },
        { id: 50, url: 'https://smile-craft-dental-care-landing-pag.vercel.app', image: 'https://itsdentaltime.ca/wp-content/uploads/2024/02/Dental-Lavelle-Why-you-need-to-visit-your-Dentist-every-6-months.jpg', title: 'Healthcare Website' },
>>>>>>> 71d059c26bce3123d9620e9d947d421e4f051f5b
      ]
    }
  ];
...
=======
  const handleToggleExpand = (e: React.MouseEvent, memberId: number) => {
    e.stopPropagation();
    setExpandedMember(expandedMember === memberId ? null : memberId);
  };
...
=======
  ...
  const ScrollingPosters = ({ posters, speed, isDeveloper = false, animationKey }: {
    posters: Array<{ id: number; url: string; title: string; image?: string }>;
    speed: number;
    isDeveloper?: boolean;
    animationKey: string;
  }) => {
...
=======
    // Using CSS keyframes for a smooth, performant, and continuous animation.
    // This approach avoids JavaScript-based animation glitches.
    const styles = `
      .scroll-container-${animationKey}:hover .scroll-content-${animationKey} {
...
<<<<<<< HEAD
=======
    const PosterContent = ({ poster }: { poster: any }) => (
      <div className="flex-shrink-0 group" style={{ width: posterWidth }}>
        <div className="w-full h-56 bg-gradient-to-br from-primary-500 to-primary-600 shadow-lg overflow-hidden relative rounded-xl">
          <img src={isDeveloper ? poster.image : poster.url} alt={poster.title} className="w-full h-full object-cover group-hover:brightness-110 transition-all duration-300 rounded-xl" />
          <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-20 transition-all duration-300 flex items-end rounded-xl">
            <div className="p-4 text-white opacity-0 group-hover:opacity-100 transition-opacity duration-300">
              <p className="font-medium text-sm">{poster.title}</p>
            </div>
...
=======
              {duplicatedPosters.map((poster, index) =>
                isDeveloper ? (
                  <a key={`${poster.id}-${index}`} href={poster.url} target="_blank" rel="noopener noreferrer" className="cursor-pointer">
                    <PosterContent poster={poster} />
                  </a>
                ) : (
                  <div key={`${poster.id}-${index}`}>
                    <PosterContent poster={poster} />
                  </div>
                )
              )}
...
=======
      <section className="relative py-20 overflow-hidden">
        <div className="absolute inset-0"><img src={pageImages.heroBackground} alt="Team background" className="w-full h-full object-cover" /><div className="absolute inset-0 bg-black/60"></div></div>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <motion.h1 className="text-5xl md:text-6xl font-bold text-white mb-6 relative z-10" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>Meet the <span className="bg-gradient-to-r from-primary-400 to-primary-600 bg-clip-text text-transparent">Creative Minds</span></motion.h1>
          <motion.p className="text-xl text-gray-200 max-w-3xl mx-auto relative z-10" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.2 }}>The talented individuals behind every successful project. Our team combines creativity, expertise, and passion to deliver exceptional results.</motion.p>
        </div>
      </section>
...
<<<<<<< HEAD
=======
                    {/* COMMENT: To change team member image size, update the w- and h- classes below. 
                        Current size is w-52 h-52 (approx. 20% smaller than w-64). */}
>>>>>>> 71d059c26bce3123d9620e9d947d421e4f051f5b
...
=======
          <div className="space-y-12">
            {teamMembers.map((member, index) => (
              <motion.div key={member.id} className="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: index * 0.1 }} viewport={{ once: true }}>
                <div className={`p-8 ${member.id !== 6 ? 'cursor-pointer' : ''}`} onClick={() => handleCardClick(member)}>
                  <div className="flex flex-col md:flex-row items-center md:items-start space-y-6 md:space-y-0 md:space-x-8">
                    <div className="flex-shrink-0"><div className="w-32 h-32 rounded-full overflow-hidden border-4 border-primary-500 shadow-lg"><img src={member.photo} alt={member.name} className="w-full h-full object-cover" /></div></div>
>>>>>>> 71d059c26bce3123d9620e9d947d421e4f051f5b
                    <div className="flex-grow text-center md:text-left">
                      <h3 className="text-3xl font-bold text-gray-900 mb-2">{member.name}</h3>
                      <p className="text-xl text-primary-600 font-semibold mb-4">{member.role}</p>
...
=======
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                        <div className="text-center bg-gray-50 p-4 rounded-lg"><div className="text-2xl font-bold text-primary-600">{member.posts}</div><div className="text-sm text-gray-600">Posts</div></div>
                        <div className="text-center bg-gray-50 p-4 rounded-lg"><div className="text-2xl font-bold text-primary-600">{member.videos}</div><div className="text-sm text-gray-600">Videos</div></div>
                        <div className="text-center bg-gray-50 p-4 rounded-lg"><div className="text-2xl font-bold text-primary-600">{member.videoHours}</div><div className="text-sm text-gray-600">Video Hours</div></div>
                        <div className="text-center bg-gray-50 p-4 rounded-lg"><div className="text-2xl font-bold text-primary-600">{member.industries}</div><div className="text-sm text-gray-600">Industries</div></div>
                      </div>
                      <button onClick={(e) => handleToggleExpand(e, member.id)} className="bg-gradient-to-r from-primary-500 to-primary-600 text-white px-8 py-3 rounded-full font-semibold hover:shadow-lg transform hover:scale-105 transition-all duration-300 flex items-center space-x-2 mx-auto md:mx-0">
                        <span>{expandedMember === member.id ? 'Show Less' : 'View More'}</span>
                        {expandedMember === member.id ? <ChevronUp className="h-5 w-5" /> : <ChevronDown className="h-5 w-5" />}
                      </button>
                    </div>
                  </div>
                  <AnimatePresence>
                    {expandedMember === member.id && (
                      <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.3 }} className="mt-8 pt-8 border-t border-gray-200">
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                          {/* Education, Stats, Links sections... */}
                        </div>
                      </motion.div>
                    )}
                  </AnimatePresence>
                ...
                <div className="bg-gray-50 p-6">
                  <div className="flex items-center space-x-3 mb-4">
                    <div className="w-8 h-8 rounded-full overflow-hidden border-2 border-primary-500"><img src={member.photo} alt={member.name} className="w-full h-full object-cover" /></div>
                    <div><h4 className="text-lg font-bold text-gray-900">{member.name}'s Work</h4><p className="text-primary-600 text-sm font-medium">{member.role}</p></div>
                  </div>
                  <ScrollingPosters speed={60 + index * 5} posters={member.posters} isDeveloper={member.id === 6} animationKey={`member-${member.id}`} />
                </div>
>>>>>>> 71d059c26bce3123d9620e9d947d421e4f051f5b
...
=======
      <AnimatePresence>
        {selectedMemberPhoto && (
          <motion.div className="fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center z-50 p-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setSelectedMemberPhoto(null)}>
            <motion.div className="relative" initial={{ scale: 0.7, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.7, opacity: 0 }} transition={{ type: 'spring', stiffness: 300, damping: 30 }} onClick={(e) => e.stopPropagation()}>
              <img src={selectedMemberPhoto} alt="Team member" className="max-w-[90vw] max-h-[90vh] object-contain rounded-lg shadow-2xl" />
              <button onClick={() => setSelectedMemberPhoto(null)} className="absolute -top-3 -right-3 bg-white rounded-full p-2 text-gray-800 hover:bg-gray-200 transition-transform duration-300 hover:scale-110 shadow-lg"><X className="h-6 w-6" /></button>
>>>>>>> 71d059c26bce3123d9620e9d947d421e4f051f5b
            </motion.div>
          </motion.div>
        )}
